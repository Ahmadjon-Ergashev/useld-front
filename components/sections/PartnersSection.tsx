    ...
    <>
      <section className="partners" id="partners">
          ...
          <div className="marquee-track">
            {repeated.map((p, i) => (
              <div className="partner-item" key={i}>
                {p.logo ? (
                  <img src={p.logo} alt={p.name} title={p.name} style={{ maxWidth: '100%', maxHeight: '60px', opacity: 0.6, transition: 'all 0.3s' }} />
                ) : (
        ...
        <div className="partners-note container">
          Interested in partnering with us? <a href="mailto:useldsales@gmail.com">Get in touch →</a>
        </div>
      </section>
    </>
  );
